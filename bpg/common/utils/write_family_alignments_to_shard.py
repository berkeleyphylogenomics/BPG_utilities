...
def main():
  ...
  shard_file = '/clusterfs/ohana/bpg/pfacts/phylofacts_alignments_%03d-of-%03d' \
              % (shard_number + 1, num_shards)
  print "Writing %s with alignments from %s to %s" % (shard_file,
  ...
  for family_accession in family_accessions[start:end]:
      ...
      alignment_file = os.path.join(dir, '%s.a2m' % family_accession)
      try:
        inf = open(alignment_file)
        alignment_data = inf.read()
        inf.close()
        outf.write("%s\n" % family_accession)
        outf.write(alignment_data)
        outf.write("//\n")
      except IOError:
        print "Could not write %s to the shard" % alignment_file
  outf.close()
