...
def main():
  ...
  if not tree_objects:
    ...
    if os.path.exists(tree_file):
      ...
      status = os.waitpid(p.pid, 0)[1]
      subst_tree_file = '%s_subst.ml' % bpg_accession
      if not os.path.exists(subst_tree_file):
        tree = Tree.objects.get(family__id = family_id, method='ml')
        print 'Writing substituted tree file %s...' % subst_tree_file,
        starttime = time.time()
        outf = open(subst_tree_file, 'w')
  ...
  p = subprocess.Popen(['touch', 'polish_family_20110525_done'] )
  status = os.waitpid(p.pid, 0)[1]
