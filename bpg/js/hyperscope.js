...
function nodeTip(node,description) {
  ...
  tip += "<br />Click <a href='http://phylofacts.berkeley.edu/phog/net/?sequence_id="+node+"&ortholog_type="+ortho_type;
...
function drawGraph(edge_list, node_coords, filename, taxon_scientific, taxon_common, query_id, ortholog_type) {
  ...
  for (var i = 0; i < node_count; i++) {
	...
	if ( ((nodes[i][0] == edge_list[k][0]) &&
	  ...
	  document.write("<area shape='poly' coords='"+x1+","+y1+","+x2+","+y2+","+x3+","+y3+","+x4+","+y4+"' href=\"javascript:void(0);\" lang=\"JavaScript\" title=\"\" onmouseover=\"edgeTip('"+edge_list[k][2]+"','"+nodes[i][0]+"','"+nodes[j][0]+"');return true;\" onclick=\"edgeClick('"+edge_list[k][2]+"','"+nodes[i]+"','"+nodes[j]+"');return true;\" onmouseout=\"UnTip(); return true;\" alt=\"\" />");
