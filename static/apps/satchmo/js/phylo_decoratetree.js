 ...
 * a JavaScript associative array.  Routines for
 * subfamily collapse are also included.  
 * 
...
var iconDir = 'http://phylogenomics.berkeley.edu/media/icons/';
var showTaxon = true;
...
function collapseSubfamilies(aRoot, aBook) {
	...
	function retrieveMRCA(aNode) {
		...
		ajaxObj.node = aNode;
		ajaxObj.requestFile = 'http://phylogenomics.berkeley.edu/rest/index.py/getMRCA?' + queryStr;
...
function decorateSubfamilyNode(aNode, aBook) {
	...
	if (aNode.subfamilyObj) {
		...
		if (showUniProt && aNode.hasSwissProt) {
			_label += "\n <img style=\"border: none;\" src=\"" + iconDir + "icon_swiss_12.png\" onclick=\"this.parentNode.node.tree.collapseNode(this.parentNode.node)\"" + 
				tip("sf_swissprot") + "> ";
		}
		...
		if (showUniProt && aTags["uniprot_accession"] != "") {
			if (aTags["uniprot_accession"] != "N/A") {
				...
				if (aTags["in_swissprot_f"] && aTags["in_swissprot_f"] == 1) {
					_htmlStr += "\n <img style=\"border: none;\" src=\"" + iconDir + "icon_swissprot_12.png\" alt=\"SwissProt\"> ";
				}
				_htmlStr += "</a> ";
			}
		}
