...
//spot(this.x, this.y, this.tree.canvasDiv);
				...
				labelDivTop[this.label] = this.labelDiv.style.top;
			}
			if (this.buttonDiv) {
	...
	this.onButtonClick = function() {
		if (this.descendant) {
			if (this.tree.isCollapsable) {
				//alert(this.leftId);
				//this.tree.collapseNode(this);
				//drawJsonSeq(jsonSeqGlobal, jsonObjGlobal, this.leftId);
				theSeqDiv.innerHTML = displayString[this.leftId];
			}
		} else { 
			if (this.tree.isSelectable) {
				theSeqDiv.innerHTML = displayString[this.leftId];
...
//	var treeLineHighlightColor = this.treeLineHighlightColor;
	...
	this.clearSelect = function() {
		...
		this.selectNodes = [];
		for (i=0; i<this.selectLineDivs.length; i++) {
