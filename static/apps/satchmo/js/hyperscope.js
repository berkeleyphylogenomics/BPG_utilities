...
function nodeClick(node) {
  //alert(node);
  window.open("http://phylofacts.berkeley.edu/phog/"+node+"/");
}

function edgeClick(node1,node2) {
  window.open("http://phylofacts.berkeley.edu/phog/phog/"+node1+"_"+node2+"/");
}

function nodeTip(node) {
  var genes = node_list[node]['genes_from_taxon'];
  var description = node_list[node]['description'];
  var tip = "<strong>PHOG description:</strong><br> &nbsp; &nbsp; &middot; <a href='http://phylofacts.berkeley.edu/phog/"+node+"/' target='_blank' title='Click for orthologs...'>"+node_list[node]['description']+"</a><br>";
  if (genes) {
    if (genes.length > 0) {
      tip+="<br><strong>Genes from query taxon:</strong><br>";
      for (var i = 0; i < genes.length; i++) {
	tip += " &nbsp; &nbsp; &middot; <a href='http://www.uniprot.org/uniprot/"+genes[i]+"' target='_blank' title='Click for UniProt page...'>"+genes[i]+"</a><br>";
      }
    }
    else {
  ...
  tip += "<br>Click <a href='http://phylofacts.berkeley.edu/phog/hyper/"+node;
  if (taxon != "None") {
    tip += "_"+taxon;
  }
...
function edgeTip(node1,node2) {
	str = "Click for list of interactions between proteins in "+node1+" and "+node2+".";
	bpgToolTip(str);
}

function drawGraph(node_coords, filename, taxon_id) {
