(function ($) {
    $.widget("ui.pfammap", {
        options: {
            ...
            progressBarColor: '#0e90d2',
            progressBarDoneColor: '#0e90d2',
            progressBarHeight: 4,
            progressBarTopSpacing: 6,
            domainHeight: 20,
            domains: [ ],
        ...
        refresh: function () {
            ...
            var height = Math.max(this.options.domainHeight + this.options.progressBarTopSpacing + this.options.progressBarHeight + 2, this.options.queryLineHeight + this.options.progressBarTopSpacing + this.options.progressBarHeight + 2);
            ...
            for(i=0; i<this.options.domains.length; i++) {
                ...
                var rect2 = this.paper.rect(this.options.domains[i].alignment_from * scale,
                    ...
                    - this.options.domains[i].alignment_from) * scale,
                    this.options.domainHeight, this.options.domainBorderRadius).attr({
                        'fill-opacity': 0, 'stroke-opacity': 0, 'cursor':'pointer',
                        'fill': 'black', 'stroke':'black'
                    }).toFront();
               
                if (this.options.domains[i].percent != 0) {var opacity = 1;}
                else {var opacity = 0;}
 
                var progressOutlineRect = this.paper.rect(this.options.domains[i].alignment_from * scale,
                    this.options.domainHeight + this.options.progressBarTopSpacing + Math.floor(this.options.progressBarHeight/2),
                    (this.options.domains[i].alignment_to - this.options.domains[i].alignment_from) * scale,
                    this.options.progressBarHeight).attr({
                        'fill-opacity':0, 'stroke': this.options.progressBarColor, 'cursor': 'default',
                        'stroke-opacity': opacity
                    });
                    
                var progressRect = this.paper.rect(this.options.domains[i].alignment_from * scale,
                    this.options.domainHeight + this.options.progressBarTopSpacing + Math.floor(this.options.progressBarHeight/2), 
                    Math.floor(((this.options.domains[i].percent)/100)*(this.options.domains[i].alignment_to - this.options.domains[i].alignment_from) * scale), 
                    this.options.progressBarHeight).attr({
                        'fill': this.options.progressBarColor, 'stroke': this.options.progressBarColor, 
                        'cursor':'default', 'fill-opacity': opacity, 'stroke-opacity': opacity
                    }).toFront();
...
(function ($) {
    $.widget("ui.mdamap", {
        ...
        refresh: function () {
            ...
            this.paper.clear();
            var width = $(this.element[0]).width() - 10; 
            var height = this.options.domainHeight;
            var scale = width / this.options.masterLength; 
            var domainY = Math.floor(height/2) - Math.floor(this.options.domainHeight/2);

            this.paper.setSize(width + 10, height);
            // Draw the query guide line
            var rect = this.paper.rect(0, domainY, width, this.options.domainHeight).attr({ fill: this.options.mdaColor, stroke: this.options.mdaColor, 'cursor':'pointer' });
            rect.mouseover(this.options.domainMouseover(this.options.domain[0]));
            rect.mouseup(this.options.domainMouseup(this.options.domain[0]));
            rect.mouseout(this.options.domainMouseout(this.options.domain[0]));
            rect.click(this.options.domainClick(this.options.domain[0]));
            this.options.domainCallback(this.options.domain[0], rect);
