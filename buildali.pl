...
my $dbbase="/clusterfs/ohana/external/ncbi/nr/current/nr"; # will use PSI-BLAST dbs $dbbase."90" and $dbbase."70" 
my $hh="/clusterfs/ohana/software/bin";
my $perl=$hh;
my $ncbidir="/usr/bin"; # directory with ncbi BLAST binaries
my $blastpgp=$ncbidir."/blastpgp -I T -s T"; # blastpgp executable
print "blastpgp: $blastpgp\n";
our $blastpgp = $blastpgp . " -I T -s T"; # show gi's in defline; use Smith-Waterman
print "Now blastpgp: $blastpgp\n";
my $execdir="/clusterfs/ohana/software/psipred261/bin";  # directory with PSIPRED binaries 
my $datadir="/clusterfs/ohana/software/psipred261/data"; # directory with PSIPRED data 
...
$qseq=~tr/a-zA-Z//cd;
$qseq=~tr/BZbzJOjo/DEde/d; # remove all newlines and non-residue characters
...
#		push(@seq,"$aa_pred");
      ...
      &System("mv $tmp$dotn.bla $outbase$dotn.bla_for_psipred");
      &System("mv $tmp$dotn.mtx $outbase$dotn.mtx");
      &System("mv $tmp$dotn.ss $outbase$dotn.ss");
      &System("mv $tmp$dotn.ss2 $outbase$dotn.ss2");
      &System("mv $tmp$dotn.horiz $outbase$dotn.horiz");
...
sub RunPsipred() {
    ...
    &System("$blastpgp -b 1 -j 1 -h 0.001 -d $dummydb -i $infile -B $basename.psi -C $basename.chk &> $basename.bla");
...
#    system ("rm -f $basename.pn $basename.sn $basename.mn $basename.bla $basename.mtx $basename.aux $basename.ss $basename.ss2");
    system ("rm -f $basename.pn $basename.sn $basename.mn $basename.aux");
