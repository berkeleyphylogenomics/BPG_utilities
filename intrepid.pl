...
sub printResult{
  ...
  my $con_js_rankFile ="con_js.rank";
  ...
  my %con_js_rank;
    ...
    (my $residue)=$auxMap{$pdbPos}=~/\|(\S)\|/;
    push (@{$con_js{$originalSeqPos}},($residue,$colorArray[1]));	
	}
	close AUXFILE;
    ...
    $con_js_rank{$originalSeqPos}=$colorArray[1];	
    
	}
	close RANKFILE;
   
  #print the rank of con_js_map  
  open (CON_JS,  ">$con_js_rankFile");
  printf CON_JS "\t%5s\t%7s\t%10s\t%9s\n","#Rank","Residue","Con_js_map","#Position";
  foreach my $orig_pos (sort {$con_js_rank{$a}<=>$con_js_rank{$b}} keys %con_js_rank)
  {
        #print $con_js_rank{$orig_pos},"\n";
        #my $print_list;
        #$print_list=join("\t",@{$con_js_rank{$rank}});
        #print $print_list,"\n";
        {printf CON_JS
"\t%-5d\t%-7s\t%5.3f\t%11d\n",$con_js_rank{$orig_pos},$con_js{$orig_pos}[0],$con_js{$orig_pos}[1],$orig_pos;}
  }
  close (CON_JS);
}
