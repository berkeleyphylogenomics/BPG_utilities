...
mafft_in_path = "%s/concat_for_multipw/%s/%s_concatenated.a2m" \
                  % (cur_dir(), pair_id, pair_id)
out_dir = "%s/mafft-multipw/%s" % (results_dir(), pair_id)
make_dir_exist(out_dir)
# Align this concatenation of the two MSAs with MAFFT.
mafft_out_path = os.path.join(out_dir, "%s_mafft-multi.afa" % pair_id)
mafft_cmd = "mafft --fft --retree 2 --maxiterate 0 --bl 62 " + \
  "--op 1.530000 --ep 0.123000"
os.system("%s %s > %s" % (mafft_cmd, mafft_in_path, mafft_out_path))
# Extract the pairwise alignment of the seeds from the new MSA.
seedX_seq = ""
seedY_seq = ""
for record in SeqIO.parse(open(mafft_out_path, "rU"), "fasta"):
...
pw_alignment_path = os.path.join(out_dir, "%s_mafftmultipw.afa" % pair_id)
"""
...
reference_alignment_path = get_reference_alignment_path(pair_id)
if os.path.exists(reference_alignment_path):
  ...
  output_file = os.path.join(out_dir, "%s_mafft-multipw.csv" % pair_id)
