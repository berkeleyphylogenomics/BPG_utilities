...
def main():
  ...
  Parser = OptionParser('usage: run_discern_on_seed.py <-s seed> <-p pdb_id> <-c comparative model>')
  ...
  if CmdLineOps.pdb_ID:
    os.system("cp /clusterfs/ohana/external/pdb_structures/%s/pdb%s.ent.gz %s.pdb.gz" 
            % (pdb[1:3], pdb, pdb))
