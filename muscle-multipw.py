...
muscle_in_path = "%s/concat_for_multipw/%s/%s_concatenated.a2m" \
                  % (cur_dir(), pair_id, pair_id)
out_dir = "%s/muscle-multipw/%s" % (shmm_root_dir(), pair_id)
make_dir_exist(out_dir)
# Align this concatenation of the two MSAs with MUSCLE.
muscle_out_path = os.path.join(out_dir, "%s_musclemulti.msa" % pair_id)
os.system("muscle -in %s -out %s -maxiters 3" \
          % (muscle_in_path, muscle_out_path))
# Extract the pairwise alignment of the seeds from the new MSA.
seedX_seq = ""
seedY_seq = ""
for record in SeqIO.parse(open(muscle_out_path, "rU"), "fasta"):
...
pw_alignment_path = os.path.join(out_dir, "%s_musclemultipw.afa" % pair_id)
"""
...
if os.path.exists(reference_alignment_path):
  ...
  output_file = os.path.join(out_dir, "%s_muscle-multipw.csv" % pair_id)
